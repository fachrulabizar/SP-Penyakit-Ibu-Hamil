...
if (!$q_idbalas) {
    die('Kueri id_balas tidak valid: ' . mysql_error());
}
$qidbalas = mysql_fetch_array($q_idbalas);

$q_idpesan = mysql_query("SELECT MAX(id_pesanm) AS id_pesanku FROM pesanm WHERE id_member='" . $rm['id_member'] . "'");
if (!$q_idpesan) {
    die('Kueri id_pesan tidak valid: ' . mysql_error());
}
