...
if(isset($_POST["periksa"])){
    ...
    for($i=1; $i<=$rtotal; $i++){
      ...
      $nnama= $_POST['nama'.$i];
            ...
            $querykonsul=mysql_query("INSERT INTO diagnosa (id_member,no_konsul,kd_gejala,cf,tanggal) VALUES ('$idmember','$no_konsul','$ngejala','$ncf','$date')");


            }else if(empty($ngejala)||empty($ncf)){

              echo"<meta http-equiv=Refresh content=2;url=?r=diagnosa>";
            ...
            if($querykonsul){
              echo "<div align='center'class='alert alert-success'>
                          <strong>Berhasil Diagnosa!
                            </div>";
              echo "<meta http-equiv=Refresh content=1;url=?r=rule&no_konsul=$no_konsul>";
            }else{
              echo "<div align='center'class='alert alert-danger'>
                          <strong>Silahkan isi data  diagnosa dengan benar!
                            </div>";
              echo "<meta http-equiv=Refresh content='1; url=?r=diagnosa'>";
            }
...
<a  href='?r=hasilkonsul'class='btn btn-info' > <span class='glyphicon glyphicon-list'></span> LIHAT HASIL DIAGNOSA SEBELUMNYA</a>
<center style='font-family:GreyscaleBasic;font-weight:bold;'><h3><b>SILAHKAN PILIH GEJALA YANG ANDA RASAKAN SAAT INI</b></h3></center>
<form action="" method="post">
  <table class= 'table table-bordered' >
    <tr>
      <td class="text-center" width="25"><b>No</b></td>
      <td class="text-center"><b>Pertanyaan</b></td>
      <td class="text-center" width="300"><b>Pilih Seberapa Besar Gejala Yang anda Miliki dari 0 - 1</b></td>
    </tr>
    <?php
                $no=1;
...
function enabletext(id) {
      console.log(id);
        if ($('#check'+id).is(':checked', true)) {
          ...
          <div align="center">
            <input type="submit" class="btn btn-info" name="periksa" id="periksa" value="Periksa" />
